...
use monadify::kind_based::kind::{ResultKind, OptionKind, VecKind, CFnKind, CFnOnceKind}; // ...HKTMarker to ...Kind
...
mod result_kind_monad_laws { // Renamed module
    use super::*;

    // 1. Left Identity: ResultKind::pure(a).bind(f) == f(a)
    #[test]
    fn result_kind_monad_left_identity_ok() { // Renamed test
        let a: i32 = 10;
        let f = clone_fn(|x: i32| -> TestResult<String> { Ok((x * 2).to_string()) });

        let lhs = ResultKind::<TestError>::bind( // Renamed Marker
    ...
    #[test]
    fn result_kind_monad_left_identity_f_returns_err() { // Renamed test
        let a: i32 = 10;
    ...
    #[test]
    fn result_kind_monad_right_identity_ok() { // Renamed test
    ...
    #[test]
    fn result_kind_monad_right_identity_err() { // Renamed test
    ...
    #[test]
    fn result_kind_monad_associativity_all_ok() { // Renamed test
        let m: TestResult<i32> = Ok(10);
    ...
    #[test]
    fn result_kind_monad_associativity_m_is_err() { // Renamed test
        let m: TestResult<i32> = Err("m_error".to_string());
        let f = clone_fn(|x: i32| -> TestResult<f64> { Ok((x * 2) as f64) });
        let g = clone_fn(|y: f64| -> TestResult<String> { Ok(y.to_string()) });
    ...
    #[test]
    fn result_kind_monad_associativity_f_returns_err() { // Renamed test
        let m: TestResult<i32> = Ok(10);
    ...
    #[test]
    fn result_kind_monad_associativity_g_returns_err() { // Renamed test
        let m: TestResult<i32> = Ok(10);
        let f = clone_fn(|x: i32| -> TestResult<f64> { Ok((x * 2) as f64) });
    ...
    #[test]
    fn result_kind_monad_join_law1() { // Renamed test
        let x = 10;
    ...
    #[test]
    fn result_kind_monad_join_law1_outer_err() { // Renamed test
    ...
    #[test]
    fn result_kind_monad_join_law1_inner_err() { // Renamed test
        let mma: TestResult<TestResult<i32>> = 
    ...
    #[test]
    fn result_kind_monad_join_law2() { // Renamed test
    ...
    #[test]
    fn result_kind_monad_join_law3() { // Renamed test
        let m_ok: TestResult<i32> = Ok(10);
        let m_err: TestResult<i32> = Err("m_error".to_string());
...
mod vec_kind_monad_laws { // Renamed module
    ...
    #[test]
    fn vec_kind_monad_left_identity_f_returns_empty() { // Renamed test
        let a: i32 = 10;
    ...
    #[test]
    fn vec_kind_monad_right_identity_non_empty() { // Renamed test
    ...
    #[test]
    fn vec_kind_monad_right_identity_empty() { // Renamed test
        let m: Vec<i32> = vec![];
        let pure_fn = clone_fn(|val: i32| VecKind::pure(val)); // Renamed Marker
    ...
    // 3. Associativity: m.bind(f).bind(g) == m.bind(|x| f(x).bind(g))
    #[test]
    fn vec_kind_monad_associativity() { // Renamed test
        ...
        let lhs = VecKind::bind( // Renamed Marker
    ...
    #[test]
    fn vec_kind_monad_associativity_empty_start() { // Renamed test
        let m: Vec<i32> = vec![];
        let f = clone_fn(|x: i32| -> Vec<i32> { vec![x, x * 10] });
        let g = clone_fn(|y: i32| -> Vec<String> { vec![y.to_string(), (y + 1).to_string()] });
        ...
        let f_inner = f.clone();
        let g_inner = g.clone();
        let composed_func = clone_fn(move |x: i32| VecKind::bind(f_inner.clone()(x), g_inner.clone())); // Renamed Marker
        let rhs = VecKind::bind(m.clone(), composed_func); // Renamed Marker
    ...
    #[test]
    fn vec_kind_monad_associativity_f_returns_empty() { // Renamed test
        let m: Vec<i32> = vec![1, 2];
    ...
    #[test]
    fn vec_kind_monad_associativity_g_returns_empty() { // Renamed test
        let m: Vec<i32> = vec![1, 2];
        let f = clone_fn(|x: i32| -> Vec<i32> { vec![x, x * 10] });
    ...
    #[test]
    fn vec_kind_monad_join_law1() { // Renamed test
        let x = 10; 
    ...
    #[test]
    fn vec_kind_monad_join_law2() { // Renamed test
    ...
    #[test]
    fn vec_kind_monad_join_law3() { // Renamed test
        let m_non_empty: Vec<i32> = vec![10, 20]; 
        let m_empty: Vec<i32> = vec![];         
    ...
    #[test]
    fn vec_kind_join_specific_examples() { // Renamed test
        assert_eq!(VecKind::join(vec![vec![1, 2], vec![3, 4]]), vec![1, 2, 3, 4]); // Renamed Marker
        ...
        assert_eq!(VecKind::join(vec![Vec::<i32>::new(), Vec::<i32>::new()]), Vec::<i32>::new()); // Renamed Marker
    }
}

mod cfn_kind_monad_laws { // Renamed module
    use super::*; 
    type Env = i32;

    // 1. Left Identity: CFnKind::pure(a).bind(f) == f(a)
    #[test]
    fn cfn_kind_monad_left_identity() { // Renamed test
        let env_val: Env = 5;
        let a: i32 = 10; 
    ...
    #[test]
    fn cfn_kind_monad_right_identity() { // Renamed test
        let env_val: Env = 7;
    ...
    // 3. Associativity: m.bind(f).bind(g) == m.bind(|x| f(x).bind(g))
    #[test]
    fn cfn_kind_monad_associativity() { // Renamed test
        let env_val: Env = 3;
        let m_creator = || CFn::new(move |env: Env| env + 1); 

        let f = clone_fn(move |x: i32| -> CFn<Env, f64> {
            CFn::new(move |env: Env| (x * env) as f64) 
        });
    ...
    #[test]
    fn cfn_kind_monad_join_law1() { // Renamed test
        let env_val: Env = 5;
        let x: i32 = 10; 
    ...
    #[test]
    fn cfn_kind_monad_join_law2() { // Renamed test
        let env_val: Env = 7;
...
mod cfn_once_kind_monad_laws { // Renamed module
    use super::*; 
    type Env = i32;

    // 1. Left Identity: CFnOnceKind::pure(a).bind(f) == f(a)
    #[test]
    fn cfn_once_kind_monad_left_identity() { // Renamed test
        let env_val: Env = 5;
        let a: i32 = 10; 
        ...
        assert_eq!(CFnOnceKind::<Env>::bind(pure_a_for_assert, f_for_assert).call_once(env_val), "15".to_string()); // Renamed Marker
    }

    // 2. Right Identity: m.bind(CFnOnceKind::pure) == m
    #[test]
    fn cfn_once_kind_monad_right_identity() { // Renamed test
        let env_val: Env = 7;
    ...
    // 3. Associativity: m.bind(f).bind(g) == m.bind(|x| f(x).bind(g))
    #[test]
    fn cfn_once_kind_monad_associativity() { // Renamed test
        let env_val: Env = 3;
        ...
        let rhs_cfn_once: CFnOnce<Env, String> = CFnOnceKind::<Env>::bind(m_creator(), composed_func); // Renamed Marker
        ...
        assert_eq!(CFnOnceKind::<Env>::bind(bound_f_check, g_check).call_once(env_val), "15".to_string()); // Renamed Marker
    }

    // Monad::join laws for CFnOnceKind
    #[test]
    fn cfn_once_kind_monad_join_law1() { // Renamed test
        let env_val: Env = 5;
        let x: i32 = 10; 
    ...
    #[test]
    fn cfn_once_kind_monad_join_law2() { // Renamed test
        let env_val: Env = 7;
        let m_creator = || CFnOnce::new(move |env: Env| env * 3); 

        let pure_fn = |val: i32| CFnOnceKind::<Env>::pure(val); // Renamed Marker
        
        let mapped_m_cfn_once: CFnOnce<Env, CFnOnce<Env, i32>> = CFnOnceKind::<Env>::map(m_creator(), pure_fn); // Renamed Marker
