...
use monadify::kind_based::kind::{OptionKind, ResultKind, VecKind, CFnKind, CFnOnceKind}; // ...HKTMarker to ...Kind
...
#[test]
fn option_kind_applicative_law_identity() { // Renamed test and HKT to Kind
    ...
    assert_eq!(OptionKind::apply(v_some.clone(), pure_id_cfn_creator()), v_some); // Renamed Marker
    assert_eq!(OptionKind::apply(v_none.clone(), pure_id_cfn_creator()), v_none); // Renamed Marker
}

#[test]
fn option_kind_applicative_law_homomorphism() { // Renamed test and HKT to Kind
...
#[test]
fn option_kind_applicative_law_interchange() { // Renamed test and HKT to Kind
...
#[test]
fn option_kind_lift_a1_functor_identity() { // Renamed test and HKT to Kind
    let fa_some: Option<i32> = Some(10);
    let fa_none: Option<i32> = None;
    let id_fn_static = identity::<i32>;

    assert_eq!(lift_a1::<OptionKind, _, _, _>(id_fn_static, fa_some.clone()), fa_some); // Renamed Marker
    assert_eq!(lift_a1::<OptionKind, _, _, _>(id_fn_static, fa_none.clone()), fa_none); // Renamed Marker
}

#[test]
fn option_kind_lift_a1_functor_composition() { // Renamed test and HKT to Kind
    let fa_some: Option<i32> = Some(10);
    let fa_none: Option<i32> = None;
...
#[test]
fn result_kind_applicative_law_identity() { // Renamed test and HKT to Kind
    ...
    assert_eq!(ResultKind::<TestError>::apply(v_ok.clone(), pure_id_cfn_creator()), v_ok); // Renamed Marker
    assert_eq!(ResultKind::<TestError>::apply(v_err.clone(), pure_id_cfn_creator()), v_err); // Renamed Marker
}

#[test]
fn result_kind_applicative_law_homomorphism() { // Renamed test and HKT to Kind
    let x: i32 = 10;
    let f = |val: i32| val * 2;
...
#[test]
fn result_kind_applicative_law_interchange() { // Renamed test and HKT to Kind
    type A = i32;
    type B = String;
    ...
    let y_val_clone_for_rhs = y_val.clone();
    let interchange_fn_creator = || CFn::new(move |f_map_fn: CFn<A, B>| f_map_fn.call(y_val_clone_for_rhs.clone()));
    let pure_interchange_fn_wrapper_creator = || ResultKind::<TestError>::pure(interchange_fn_creator()); // Renamed Marker

    let rhs_ok = ResultKind::<TestError>::apply(u_ok_creator(), pure_interchange_fn_wrapper_creator()); // Renamed Marker
    let rhs_err = ResultKind::<TestError>::apply(u_err_creator(), pure_interchange_fn_wrapper_creator()); // Renamed Marker
...
#[test]
fn result_kind_lift_a1_functor_identity() { // Renamed test and HKT to Kind
    let fa_ok: Result<i32, TestError> = Ok(10);
    let fa_err: Result<i32, TestError> = Err("Error".to_string());
    let id_fn_static = identity::<i32>;

    assert_eq!(lift_a1::<ResultKind<TestError>, _, _, _>(id_fn_static, fa_ok.clone()), fa_ok); // Renamed Marker
    assert_eq!(lift_a1::<ResultKind<TestError>, _, _, _>(id_fn_static, fa_err.clone()), fa_err); // Renamed Marker
}

#[test]
fn result_kind_lift_a1_functor_composition() { // Renamed test and HKT to Kind
    let fa_ok: Result<i32, TestError> = Ok(10);
    let fa_err: Result<i32, TestError> = Err("Error".to_string());
...
#[test]
fn vec_kind_applicative_law_identity() { // Renamed test and HKT to Kind
    println!("NOTE: VecKind Applicative Identity law is untestable with CFn due to pure's Clone constraint.");
}

#[test]
fn vec_kind_applicative_law_homomorphism() { // Renamed test and HKT to Kind
    println!("NOTE: VecKind Applicative Homomorphism law is untestable with CFn due to pure's Clone constraint.");
}

#[test]
fn vec_kind_applicative_law_interchange() { // Renamed test and HKT to Kind
    type A = i32;
    let y_val: A = 10;
...
#[test]
fn vec_kind_functor_identity_via_map() { // Renamed test and HKT to Kind
...
#[test]
fn vec_kind_functor_composition_via_map() { // Renamed test and HKT to Kind
    let fa_vec: Vec<i32> = vec![10, 20];
    let fa_empty: Vec<i32> = vec![];
...
#[test]
fn identity_kind_applicative_law_identity() { // Renamed test and HKT to Kind
...
#[test]
fn identity_kind_applicative_law_homomorphism() { // Renamed test and HKT to Kind
    let x: i32 = 10;
    let f = |val: i32| val * 2;
...
#[test]
fn identity_kind_applicative_law_interchange() { // Renamed test and HKT to Kind
    type A = i32;
    type B = String;
...
#[test]
fn identity_kind_lift_a1_functor_identity() { // Renamed test and HKT to Kind
    let fa_id: IdType<i32> = IdType(10);
    let id_fn_static = identity::<i32>;

    assert_eq!(lift_a1::<IdentityKind, _, _, _>(id_fn_static, fa_id.clone()), fa_id); // Renamed Marker
}

#[test]
fn identity_kind_lift_a1_functor_composition() { // Renamed test and HKT to Kind
    let fa_id: IdType<i32> = IdType(10);
...
#[test]
fn cfn_kind_applicative_law_identity() { // Renamed test and HKT to Kind
    println!("NOTE: CFnKind Applicative Identity law is untestable with CFn due to pure's Clone constraint.");
}

#[test]
fn cfn_kind_applicative_law_homomorphism() { // Renamed test and HKT to Kind
    println!("NOTE: CFnKind Applicative Homomorphism law is untestable due to CFn not being Clone.");
}

#[test]
fn cfn_kind_applicative_law_interchange() { // Renamed test and HKT to Kind
...
#[test]
fn cfn_kind_functor_identity_via_map() { // Renamed test and HKT to Kind
...
#[test]
fn cfn_kind_functor_composition_via_map() { // Renamed test and HKT to Kind
    let fa_creator = || CFn::new(|_e: Env| 10);
...
#[test]
fn cfn_once_kind_applicative_law_identity() { // Renamed test and HKT to Kind
...
#[test]
fn cfn_once_kind_applicative_law_homomorphism() { // Renamed test and HKT to Kind
...
#[test]
fn cfn_once_kind_applicative_law_interchange() { // Renamed test and HKT to Kind
...
#[test]
fn cfn_once_kind_functor_identity_via_map() { // Renamed test and HKT to Kind
...
#[test]
fn cfn_once_kind_functor_composition_via_map() { // Renamed test and HKT to Kind
    let fa_creator = || CFnOnce::new(|_e: Env| 10);
...
#[test]
fn reader_t_kind_applicative_law_identity() { // Renamed test and HKT to Kind
    println!("NOTE: ReaderTKind Applicative Identity law is untestable with CFn due to pure's Clone constraint.");
}

#[test]
fn reader_t_kind_applicative_law_homomorphism() { // Renamed test and HKT to Kind
    println!("NOTE: ReaderTKind Applicative Homomorphism law is untestable with CFn due to pure's Clone constraint.");
}

#[test]
fn reader_t_kind_applicative_law_interchange() { // Renamed test and HKT to Kind
...
#[test]
fn reader_t_kind_functor_identity_via_map() { // Renamed test and HKT to Kind
    let fa_creator = || ReaderT::<ReaderEnv, IdentityKind, i32>::new( // Renamed Marker
...
#[test]
fn reader_t_kind_functor_composition_via_map() { // Renamed test and HKT to Kind
    let fa_creator = || ReaderT::<ReaderEnv, IdentityKind, i32>::new( // Renamed Marker
        |_e: ReaderEnv| IdType(10)
    );
